...
1. PROFILE セクションを新設（src/components/home/profile.tsx）
   - 代表者: 天地 知也
   - 一言: Trivy 初期コミッタ / Dockle 作者
   - 認定資格バッジ: CISSP, CKAD/CKA/CKS, AWS, Azure, OCI（credly リンク付き）
   - 「詳細プロフィール」リンク → /about

2. WORKS セクションを新設（src/components/home/works.tsx）
   - 主要取引先（実名 or 業界表記、確認結果に基づく）
   - 自社プロダクト: Security Issues in Popular Containers, FutureVuls

3. /about ページ新設（src/pages/about.tsx）
   - Scrapbox about_GOODWITH の本格移植
   - 略歴、資格・認定、OSS活動、執筆/インタビュー、登壇履歴

完了条件:
- npm run build 成功
- TOP ページに PROFILE/WORKS が追加されている
- /about にアクセスして詳細プロフィールが表示される
- sitemap.xml に /about が含まれる
```
